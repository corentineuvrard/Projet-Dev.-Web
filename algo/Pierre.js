...
function init()
{
	...
	document.getElementsByTagName("svg")[0].addEventListener("mousemove", function(e) { drawTracker(e, size); });
	document.getElementById("skip_button").addEventListener("click", function() { skip(); });
}
...
// Allow a player to skip
function skip()
{
	nbCoups += 1;

	var trackingStone = document.getElementsByClassName("tracker")[0];

	trackingStone.setAttribute("fill", getColor());
	trackingStone.setAttribute("stroke", getColor());
}
